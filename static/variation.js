    ...
    <div className="gallery">
      {images.map((image, index) => (
        <div key={index} className="image-container">
          <img
            src={`${getImagePath(image.url)}?v=${encodeURIComponent(image.createdAt)}`}
            className="thumbnail"
            alt={`Image ${index + 1}`}
            loading="lazy"
            decoding="async"
...
const EnlargedImage = React.forwardRef(({ images, currentIndex, onClose, onNavigate, onDownload, totalImages }, ref) => {
  ...
  const getImagePath = (url) => {
    try {
      ...
      console.error("Invalid URL:", url);
      return url; // fallback to the original url if it's not a valid URL
    }
  };

  const formatCreationTime = (timeString) => {
    // The timeString is already in the desired format, so we can return it as is
    return timeString;
  };
  ...
  return (
    <div className="enlarged-image-overlay">
      <div className="enlarged-image-container" style={{ width: dimensions.width, height: dimensions.height }}>
        ...
        >
          <img 
            src={`${getImagePath(images[currentIndex].url)}?v=${encodeURIComponent(images[currentIndex].createdAt)}`}
            alt="Enlarged image"
            loading="lazy"
            decoding="async"
            crossOrigin="anonymous"
        ...
        <div className="enlarged-image-title-container" style={{ maxWidth: textWidth, zIndex: 2 }}>
          <div className="enlarged-image-text">
            ...
            <p className="enlarged-image-subtitle">
              ...
              Seed: {images[currentIndex].seed || 'Random'} |
...
function Variate() {
  ...
  const [prompt, setPrompt] = useState(() => sessionStorage.getItem('variation_prompt') || '');
  const [images, setImages] = useState(() => {
    try {
      const savedImages = sessionStorage.getItem('variation_images');
  ...
  const [negativeTextareaHeight, setNegativeTextareaHeight] = useState('auto');
  const textareaRef = React.useRef(null);
  const negativeTextareaRef = React.useRef(null);
  const [model, setModel] = useState(() => sessionStorage.getItem('variation_model') || 'Dreamshaper');
  const [style, setStyle] = useState(() => sessionStorage.getItem('variation_style') || 'None');
  const [strength, setStrength] = useState(() => {
    const savedStrength = sessionStorage.getItem('variation_strength');
    return savedStrength ? parseFloat(parseFloat(savedStrength).toFixed(2)) : 0.55;
  });
  const [quantity, setQuantity] = useState(() => parseInt(sessionStorage.getItem('variation_quantity')) || 1);
  ...
  const [seed, setSeed] = useState(() => sessionStorage.getItem('variation_seed') || '');
  const [negativePrompt, setNegativePrompt] = useState(() => sessionStorage.getItem('variation_negative_prompt') || '');
  ...
  useEffect(() => {
    ...
    fetch('/v1/presets/legacy/variation')
      .then(response => response.json())
  ...
  // Add a new function to safely update session storage
  ...
  useEffect(() => {
    try {
      if (prompt) updateSessionStorage('variation_prompt', prompt);
      if (negativePrompt) updateSessionStorage('variation_negative_prompt', negativePrompt);
      if (images.length > 0) updateSessionStorage('variation_images', images);
      if (model) updateSessionStorage('variation_model', model);
      if (style) updateSessionStorage('variation_style', style);
      if (quantity) updateSessionStorage('variation_quantity', quantity);
      if (strength) updateSessionStorage('variation_strength', strength);
      if (seed) updateSessionStorage('variation_seed', seed);
    } catch (error) {
      console.error('Error updating session storage:', error);
    }
  }, [prompt, negativePrompt, images, model, style, quantity, strength, seed]);

  // Update cleanup effect
  useEffect(() => {
    return () => {
      try {
        const storedImages = sessionStorage.getItem('variation_images');
        if (storedImages) {
          const parsedImages = JSON.parse(storedImages);
          if (!Array.isArray(parsedImages)) {
            console.warn('Invalid images in storage, clearing...');
            sessionStorage.removeItem('variation_images');
          }
        }
      } catch (error) {
  ...
  const generateImages = async () => {
    ...
    try {
        ...
        const costPerImage = costs && costs.variation;
        ...
        if (data.credits < totalCost) {
            ...
            throw new Error(
              ...
              `You need ${neededCredits} more credits to create ${quantity} variation${quantity > 1 ? 's' : ''}.\n` +
        ...
        const generateSingleImage = async (index) => {
            try {
                ...
                formData.append('prompt', prompt.trim() || 'No Prompt Provided');
                ...
                const response = await fetch('/v1/legacy/variation', {
                    method: 'POST',
                    body: formData
                });
                ...
                if (data.image_url) {
                    ...
                    updateSessionStorage('variation_images', newImages);
                } else {
  ...
  const handleSeedChange = (e) => {
    ...
    setSeed(newValue);
    if (newValue === '') {
      sessionStorage.removeItem('variation_seed');
    } else {
      sessionStorage.setItem('variation_seed', newValue);
    }
  };
  ...
  return (
    <div className="container">
      ...
      <div className="content">
        <RainbowText text="Atelier Variation" isAnimating={isGenerating} />
          ...
          <div className="select-container">
            ...
            <input
              ...
              onChange={(e) => {
                ...
                const value = Math.min(Math.max(0.50, parseFloat(e.target.value)), 1.00);
                if (!isNaN(value)) {
                  setStrength(value);
                }
              }}
        ...
        <button 
          ...
          className={`variate-button ${!selectedFile ? 'disabled' : ''}`}
        >
          {isGenerating ? (
            <React.Fragment>
              <i className="fas fa-gear fa-spin" style={{ marginRight: '8px' }} />
              Variating... (Do not leave or refresh the page!)
            </React.Fragment>
          ) : 'Variate Image'}
        </button>
        {error && (
...
const styles = `
  ...
  .variate-button:hover:not(:disabled) {
    background-color: #4fa8d5;
  }

  .variate-button:disabled {
