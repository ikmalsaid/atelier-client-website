...
function Generator() {
  ...
  const [prompt, setPrompt] = useState(() => sessionStorage.getItem('generation_prompt') || '');
  const [images, setImages] = useState(() => {
    try {
      const savedImages = sessionStorage.getItem('generation_images');
  ...
  const [size, setSize] = useState(() => sessionStorage.getItem('generation_size') || '1:1');
  const [model, setModel] = useState(() => sessionStorage.getItem('generation_model') || 'Turbo');
  const [style, setStyle] = useState(() => sessionStorage.getItem('generation_style') || 'None');
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const dropdownRef = React.useRef(null);
  const [textareaHeight, setTextareaHeight] = useState('auto');
  const textareaRef = React.useRef(null);
  const [quantity, setQuantity] = useState(() => parseInt(sessionStorage.getItem('generation_quantity')) || 1);
  const [sizeOptions, setSizeOptions] = useState([]);
  const [modelOptions, setModelOptions] = useState([]);
  const [styleOptions, setStyleOptions] = useState([]);
  const [costs, setCosts] = useState(null);
  const enlargedImageRef = React.useRef(null);
  const [negativePrompt, setNegativePrompt] = useState(() => sessionStorage.getItem('generation_negative_prompt') || '');
  const negativeTextareaRef = React.useRef(null);
  const [showScrollTop, setShowScrollTop] = useState(false);
  const [seed, setSeed] = useState(() => sessionStorage.getItem('generation_seed') || '');
  ...
  useEffect(() => {
    ...
    fetch('/v1/presets/sizes')
      .then(response => response.json())
      .then(data => setSizeOptions(data.sizes))
      .catch(error => console.error('Error fetching sizes:', error));

    fetch('/v1/presets/models')
      .then(response => response.json())
  ...
  const updateSessionStorage = (key, value) => {
    try {
      if (value === null || value === undefined) {
        sessionStorage.removeItem(`generation_${key}`);
      } else {
        const serializedValue = typeof value === 'object' ? JSON.stringify(value) : value.toString();
        sessionStorage.setItem(`generation_${key}`, serializedValue);
      }
    } catch (error) {
      console.error(`Error updating session storage for key generation_${key}:`, error);
    }
  };
  ...
  useEffect(() => {
    return () => {
      try {
        const storedImages = sessionStorage.getItem('generation_images');
        if (storedImages) {
          const parsedImages = JSON.parse(storedImages);
          if (!Array.isArray(parsedImages)) {
            console.warn('Invalid images in storage, clearing...');
            sessionStorage.removeItem('generation_images');
          }
        }
      } catch (error) {
  ...
  const generateImages = async () => {
    ...
    try {
        ...
        const costPerImage = costs && costs.generate;
        ...
        const generateSingleImage = async (index) => {
            try {
                ...
                const response = await fetch('/v1/legacy/generate', {
                    method: 'POST',
                    body: formData
                });
  ...
  const handleSeedChange = (e) => {
    ...
    setSeed(newValue);
    if (newValue === '') {
      sessionStorage.removeItem('generation_seed');
    } else {
      sessionStorage.setItem('generation_seed', newValue);
    }
  };
  ...
  return (
    <div className="container">
      ...
      <div className="content">
        <RainbowText text="Atelier Generator" isAnimating={isGenerating} />
